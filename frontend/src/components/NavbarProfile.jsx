...
export default function NavbarProfile() {
    ...
    if (!isLoggedIn) {
        ...
        return (
            <Link
                href="/login"
                className={`${accesoStyle} px-3 sm:px-5 md:px-6 py-2 sm:py-2.5 rounded-full text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider shrink-0`}
            >
                ACCESO
            </Link>
        );
        ...
        <div className="relative flex items-center gap-3" ref={menuRef}>
            {showCreateButton && (
                <button
                    ...
                    className="hidden md:flex h-11 w-11 items-center justify-center rounded-full border border-violet-400/40 bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-[0_0_18px_rgba(168,85,247,0.35)] transition-all hover:scale-105 active:scale-95"
                >
            ...
            {isMenuOpen && (
                <div className="absolute right-0 top-14 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 py-2 flex flex-col overflow-hidden z-50">
                    ...
                    <div className="md:hidden">
                        <button
                            onClick={handleCrearRecurso}
                            className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 text-left text-sm font-semibold dark:text-slate-200 text-slate-700 transition-colors w-full"
                        >
