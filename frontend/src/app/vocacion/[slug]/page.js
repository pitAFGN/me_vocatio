...
export default function VocationDetail({ params }) {
    ...
    if (!profession) {
        return (
            <main className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-center text-white font-sans">
                <h1 className="text-2xl font-bold mb-2">Vocación no encontrada</h1>
                <p className="text-indigo-200/70 text-sm mb-6">La ruta o profesión que buscas no existe o fue movida.</p>
                <button
    ...
    return (
        <main className="min-h-screen bg-[#0f172a] p-8 pt-32 relative overflow-hidden font-sans text-slate-100">
            ...
            <div className="max-w-4xl mx-auto relative z-10">
                ...
                <div className="bg-[linear-gradient(135deg,_#1e1b4b_0%,_#0f172a_100%)] border border-indigo-900/60 rounded-3xl p-8 shadow-2xl mb-8 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start gap-4 mb-4">
                        <div>
                            <span className="text-[11px] font-bold text-indigo-300 uppercase tracking-wider bg-indigo-950/80 border border-indigo-800/50 px-3.5 py-1.5 rounded-full inline-block">
                                {profession.area}
                            </span>
                            <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight mt-3 mb-2">
                                {profession.title}
                            </h1>
                        </div>
                    </div>

                    <p className="text-indigo-100/90 text-base leading-relaxed font-medium mb-8 max-w-2xl">
                        {profession.desc}
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-indigo-900/40">
                        <div className="flex items-start gap-3 bg-slate-900/40 p-4 rounded-2xl border border-indigo-950">
                            ...
                            <div>
                                <h4 className="text-[10px] font-bold text-indigo-300/60 uppercase tracking-wider">Demanda Laboral</h4>
                                <p className="text-sm font-semibold text-white mt-0.5">{profession.demanda}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3 bg-slate-900/40 p-4 rounded-2xl border border-indigo-950">
                            ...
                            <div>
                                <h4 className="text-[10px] font-bold text-indigo-300/60 uppercase tracking-wider">Nivel Recomendado</h4>
                                <p className="text-sm font-semibold text-white mt-0.5">{profession.nivelRecomendado}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-slate-900/60 border border-indigo-950 rounded-2xl p-6 shadow-md backdrop-blur-sm">
